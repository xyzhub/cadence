...
// Also refreshes cadence-overview.html if it already exists (opt-in progress dashboard).
...
// ── progress reporting: keep the overview dashboard fresh once it exists ────────
// Opt-in by presence — generate it once (`overview.mjs --open`) and every tick refreshes
// it. Best-effort: never creates one unprompted, never fails the tick.
if (existsSync(join(DIR, 'cadence-overview.html'))) {
  if (run('overview.mjs').status === 0) console.log('\n↻ refreshed cadence-overview.html')
}
