...
// Cargo, go.mod) at the root, in depth-1 subprojects, AND in workspace packages
// (package.json "workspaces" / pnpm-workspace.yaml; packages|apps|libs|services/*) —
// monorepo gates get cwd set. Writes .cadence/{config,ledger,lib,.gitignore}; appends
// an AGENTS.md contract block. Never clobbers an existing config or ledger.
//
//   node adopt.mjs [--root <dir>] [--goal "<text>"] [--dry-run]
//
// --dry-run detects gates and prints what it WOULD write, touching nothing —
// a safe preview for adopting an existing repo.
//
...
const CONTAINER_DIRS = ['packages', 'apps', 'libs', 'services'] // workspace containers to descend even without a manifest
const SUBGATE_CAP = 16
const IDENT = /^[A-Za-z0-9._-]+$/
// immediate child directory NAMES of `dir` that are clean identifiers, sorted (deterministic).
const childDirs = (dir) => { try { return readdirSync(dir, { withFileTypes: true }).filter(e => e.isDirectory() && !e.name.startsWith('.') && IDENT.test(e.name)).map(e => e.name).sort() } catch { return [] } }

// Declared workspace package globs (package.json "workspaces" + pnpm-workspace.yaml).
function workspaceGlobs (root) {
  const globs = []
  const pkg = read(join(root, 'package.json'))
  if (pkg) { try { const ws = JSON.parse(pkg).workspaces; if (Array.isArray(ws)) globs.push(...ws); else if (ws && Array.isArray(ws.packages)) globs.push(...ws.packages) } catch { /* unparseable */ } }
  const yaml = read(join(root, 'pnpm-workspace.yaml'))
  if (yaml) { for (const ln of yaml.split('\n')) { const m = ln.match(/^\s*-\s*['"]?([^'"#\n]+?)['"]?\s*(?:#.*)?$/); if (m) globs.push(m[1].trim()) } } // tolerate a trailing # comment
  return globs
}
// Resolve a SIMPLE glob to dirs relative to root: "dir/*" (one level) or a literal "dir". Deeper globs are skipped.
function resolveGlob (root, glob) {
  glob = glob.replace(/^\.\//, '').replace(/\/+$/, '')
  if (glob.endsWith('/*')) { const base = glob.slice(0, -2); return childDirs(join(root, base)).map(n => `${base}/${n}`) }
  if (glob.includes('*')) return [] // deeper/complex globs unsupported — add by hand
...
function detect () {
  ...
  // 2) collect candidate subproject dirs (relative to ROOT), deduped, in priority order:
  //    DECLARED workspace packages first (they own the cap budget), then depth-1 dirs, then
  //    children of container dirs (packages/apps/libs/services) that aren't projects themselves.
  //    `!`-globs resolve to an exclusion set applied to EVERY candidate (both paths).
  const wsGlobs = workspaceGlobs(ROOT)
  const excluded = new Set(wsGlobs.filter(g => g.startsWith('!')).flatMap(g => resolveGlob(ROOT, g.slice(1))))
  const candidates = []; const seen = new Set()
  const consider = (rel) => { if (rel && !seen.has(rel) && !excluded.has(rel)) { seen.add(rel); candidates.push(rel) } }
  for (const g of wsGlobs) { if (!g.startsWith('!')) for (const rel of resolveGlob(ROOT, g)) consider(rel) }
  for (const name of childDirs(ROOT)) { if (!SKIP_DIRS.has(name)) consider(name) }
  for (const c of CONTAINER_DIRS) { if (!Object.keys(gatesForDir(join(ROOT, c)).gates).length) for (const k of childDirs(join(ROOT, c))) consider(`${c}/${k}`) }

  // 3) emit cwd gates for each candidate that is actually a project, with collision-safe ids
  const subProjects = []; let subGates = 0; let capped = false
  for (const rel of candidates) {
    if (capped) break
    const sub = gatesForDir(join(ROOT, rel)) // subprojects need an explicit Python marker (no bare-tests/) [S1]
    if (!Object.keys(sub.gates).length) continue
    subProjects.push(rel)
    const leaf = rel.split('/').pop()
    for (const [id, g] of Object.entries(sub.gates)) {
      if (subGates >= SUBGATE_CAP) { capped = true; break }
      let key = `${leaf}-${id}`
      if (gates[key]) key = `${rel.replace(/[^A-Za-z0-9._-]+/g, '-')}-${id}` // leaf collision -> full-path prefix
      gates[key] = { ...g, cwd: rel }
      subGates++
    }
  }
...
// ── dry run ─────────────────────────────────────────────────────────────────
// Detect + report what adopt WOULD do, reflecting current on-disk state (so the
// preview matches adopt's idempotent "leave untouched" behavior), then exit before
// any side effect. Reuses detect()/read()/offer() — no separate code path to drift.
function reportDryRun () {
  const { gates, notes } = detect()
  const cfgP = join(CDIR, 'cadence.config.json')
  const giP = join(CDIR, '.gitignore')
  const ledP = join(CDIR, 'loop-state.json')
  const agP = join(ROOT, 'AGENTS.md'); const clP = join(ROOT, 'CLAUDE.md')
  const ag = read(agP); const cl = read(clP)
  console.log('Cadence adopt — DRY RUN (detection only; nothing will be written)\n')
  console.log('• would copy core scripts + schemas -> .cadence/lib/')
  console.log(existsSync(giP) ? '• .cadence/.gitignore exists — would leave untouched' : '• would write .cadence/.gitignore (ignores volatile loop state)')
  if (existsSync(cfgP)) {
    console.log('• .cadence/cadence.config.json exists — would leave untouched')
  } else {
    console.log(`• would write .cadence/cadence.config.json  (${notes.join('; ') || 'no gates auto-detected — you would edit it'})`)
    console.log(`  gates: ${Object.keys(gates).length ? Object.keys(gates).join(', ') : 'TODO (none auto-detected)'}`)
  }
  if (existsSync(ledP)) console.log('• .cadence/loop-state.json exists — would leave untouched (run state safe)')
  else console.log(GOAL ? `• would seed .cadence/loop-state.json with goal "${GOAL}" + a plan-backlog item` : '• would seed an empty .cadence/loop-state.json (pass --goal to seed a planning tick)')
  if (ag && ag.includes('<!-- cadence:start -->')) console.log('• AGENTS.md already has the Cadence block — would leave untouched')
  else console.log(ag ? '• would append the Cadence block to AGENTS.md' : '• would create AGENTS.md with the Cadence block')
  if (cl && /@AGENTS\.md/.test(cl)) console.log('• CLAUDE.md already imports @AGENTS.md — would leave untouched')
  else console.log(cl ? '• would add the @AGENTS.md import to CLAUDE.md' : '• would create CLAUDE.md (thin @AGENTS.md import)')
  const ro = offer(ROOT); if (ro) console.log('\n' + ro)
  console.log('\nDRY RUN — nothing written. Re-run without --dry-run to apply.')
}
if (DRY_RUN) { reportDryRun(); process.exit(0) }

// ── apply ─────────────────────────────────────────────────────────────────────
mkdirSync(join(CDIR, 'lib'), { recursive: true })
for (const f of ['ledger.mjs', 'run-gate.mjs', 'relevance.mjs', 'doctor.mjs', 'tick.mjs', 'context-budget.mjs', 'retrieval.mjs', 'overview.mjs', 'selftest.mjs']) {
...
console.log('\n✓ Cadence ready.')
console.log('Next (Claude Code): /cadence status · /cadence plan · /cadence start   (one pass: /cadence-tick · on a timer: /loop 10m /cadence-tick)')
console.log('Or raw CLI:         node .cadence/lib/doctor.mjs   then   node .cadence/lib/ledger.mjs show')
