...
function rail (s) {
  ...
  const decided = tickSet(s.recentDecisions, d => d.tick)
  const cur = s.tick
  ...
  const beats = []
  for (let t = 1; t <= cur; t++) {
    let cls = 'b-noop'; let title = `tick ${t}`
    if (green.has(t)) { cls = 'b-done'; title = `tick ${t}: shipped` }
    else if (red.has(t)) { cls = 'b-fail'; title = `tick ${t}: gate red` }
    else if (decided.has(t)) { cls = 'b-decide'; title = `tick ${t}: decision/no-op` }
    const live = (t === cur && s.inFlight) ? ' b-live' : ''
    beats.push(`<i class="beat ${cls}${live}" title="${esc(title)}"></i>`)
  }
  const legend = '<span class="lg"><i class="beat b-done"></i>shipped</span><span class="lg"><i class="beat b-fail"></i>gate red</span><span class="lg"><i class="beat b-decide"></i>no-op</span>'
...
function blockedLane (s) {
  ...
  const rows = items.map(b => `
    <div class="row blocked">
      ...
      <span class="since">since t${esc(b.since)}</span>
...
:root{
  ...
  --text:#e7ebf3;--dim:#8a93a8;--faint:#5b6477;
...
.rail{margin:16px 0 20px;padding:16px 18px;border:1px solid var(--line);border-radius:14px;
  background:var(--panel) linear-gradient(90deg,rgba(139,123,255,.05) 1px,transparent 1px);background-size:14px 100%;}
...
.beat.b-decide{height:26px;background:var(--inflight);opacity:.7} .beat.b-noop{height:16px;background:var(--line2)}
...
if (args.includes('--stdout')) { process.stdout.write(html) } else {
  ...
  if (args.includes('--open')) {
    ...
    try { spawn(opener, [out], { detached: true, stdio: 'ignore' }).unref() } catch { /* best-effort */ }
  }
}
