...
test('ledger: block→unblock preserves the item\'s full prior context (P02)', () => {
  const d = work('led-block')
  run('ledger.mjs', ['init'], { dir: d })
  run('ledger.mjs', ['add', 't1', '7', 'x', '--gate', 'test', '--brief', 'plan/b.md#t1'], { dir: d })
  run('ledger.mjs', ['begin', 't1'], { dir: d })
  ...
  eq(p.score, 7, 'score preserved (not reset to 0)')
  eq(p.gate, 'test', 'gate preserved'); eq(p.brief, 'plan/b.md#t1', 'brief preserved')
  eq(p.lastError?.firstError, 'priorboom', 'prior lastError preserved — carries its context')
})

test('ledger: unblock of an OLD-shape blocked entry (no score) defaults to 0 and stays valid', () => {
  const d = work('led-block-legacy')
  run('ledger.mjs', ['init'], { dir: d })
  const s = readLedger(d)
  s.blockedOnOwner = [{ id: 'old', desc: 'legacy', since: 0 }] // pre-fix shape: no score
  writeRaw(d, 'loop-state.json', JSON.stringify(s, null, 2))
  eq(run('ledger.mjs', ['unblock', 'old'], { dir: d }).status, 0, 'unblock exit 0')
  const p = readLedger(d).pending.find(x => x.id === 'old')
  assert(p, 'restored to pending'); eq(p.score, 0, 'score defaults to 0')
  eq(run('ledger.mjs', ['validate'], { dir: d }).status, 0, 'ledger still valid')
})
...
// The single-command case above is exec-optimized by the shell (child.kill hits the real
// process), so it can't catch the close-only-resolver hang. A PIPELINE keeps the shell as
// the parent; `sleep` survives a bare child.kill() and holds the stdout pipe, so 'close'
// never fires. This guards the process-group SIGKILL + resolve-on-timer fix. [run-gate timeout bug]
test('run-gate: a hung command whose grandchildren survive a bare kill still times out (process-group SIGKILL)', () => {
  const d = work('rg-timeout-group')
  writeConfig(d, { project: 't', gates: { slow: { cmd: 'sleep 1000 | cat', timeoutMs: 800 } } })
  const r = run('run-gate.mjs', ['slow'], { dir: d })
  eq(r.status, 1, 'exit (runner did not hang)') // r.status===null would mean it hung to the suite watchdog
  const sig = json(r.stdout)
  eq(sig.reason, 'error', 'reason')
  match(sig.firstError, /timed out after \d+ms/, 'firstError')
  assert(sig.ms < 5000, `resolved promptly at the timeout, not via the suite watchdog: ms=${sig.ms}`)
})
...
testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  ...
  assert(existsSync(join(cdir(d), 'lib', 'overview.mjs')), 'overview dashboard generator copied')
...
testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt --dry-run reports detected gates and writes nothing', () => {
  const d = work('adopt-dryrun')
  writeFileSync(join(d, 'package.json'), JSON.stringify({ name: 'p', version: '1.0.0', scripts: { test: 'jest' } }))
  const r = run('adopt.mjs', ['--dry-run', '--goal', 'preview only'], { dir: d, cwd: d })
  eq(r.status, 0, 'dry-run exit 0')
  assert(!existsSync(cdir(d)), 'dry-run creates no .cadence/ (nothing written)')
  assert(!existsSync(join(d, 'AGENTS.md')), 'dry-run does not create AGENTS.md')
  match(r.stdout, /dry run/i, 'stdout announces a dry run')
  match(r.stdout, /\btest\b/, 'stdout lists the detected gate')
})
...
// ── I. planning (Protocol 10) ──────────────────────────────────────────────────
test('ledger: add carries brief/gate/accept/phase pointers and validates', () => {
  const d = work('plan-add')
  run('ledger.mjs', ['init'], { dir: d })
  run('ledger.mjs', ['add', 'i1', '5', 'do x', '--gate', 'test', '--accept', 'x works', '--brief', 'plan/g.briefs.md#i1', '--phase', 'p1'], { dir: d })
  eq(run('ledger.mjs', ['validate'], { dir: d }).status, 0, 'ledger still valid with planning fields')
  const item = JSON.parse(run('ledger.mjs', ['next'], { dir: d }).stdout)
  eq(item.id, 'i1', 'next item'); eq(item.gate, 'test', 'gate'); eq(item.accept, 'x works', 'accept')
  eq(item.brief, 'plan/g.briefs.md#i1', 'brief pointer'); eq(item.phase, 'p1', 'phase')
})

test('ledger: add without planning flags stays minimal (no empty fields)', () => {
  const d = work('plan-add-min')
  run('ledger.mjs', ['init'], { dir: d })
  run('ledger.mjs', ['add', 'i1', '5', 'do x'], { dir: d })
  const item = JSON.parse(run('ledger.mjs', ['next'], { dir: d }).stdout)
  eq(item.brief, undefined, 'no brief key'); eq(item.gate, undefined, 'no gate key'); eq(item.phase, undefined, 'no phase key')
})

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt --goal seeds a high-score plan-backlog item (planning is tick 0)', () => {
    const d = work('adopt-goal')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ name: 'p', version: '1.0.0' }))
    eq(run('adopt.mjs', ['--goal', 'migrate to typed routes'], { dir: d, cwd: d }).status, 0, 'exit 0')
    const s = readLedger(d)
    eq(s.currentGoal, 'migrate to typed routes', 'goal recorded')
    eq(s.pending.length, 1, 'one seeded item')
    eq(s.pending[0].id, 'plan-backlog', 'it is the plan item')
    eq(s.pending[0].score, 100, 'high score so it runs first')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt without --goal seeds an empty backlog (no plan item)', () => {
    const d = work('adopt-nogoal')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ name: 'p', version: '1.0.0' }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const s = readLedger(d)
    eq(s.pending.length, 0, 'empty backlog when no goal')
  })

// ── J. adopt — monorepo subprojects + gitignore (live-dogfood fixes) ───────────
testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: detects a depth-1 subproject gate with cwd set (monorepo, no root package.json)', () => {
    const d = work('adopt-monorepo')
    mkdirSync(join(d, 'api'), { recursive: true })
    writeFileSync(join(d, 'api', 'package.json'), JSON.stringify({ name: 'api', scripts: { test: 'echo ok' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const cfg = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8'))
    eq(cfg.gates['api-test']?.cwd, 'api', 'subproject gate carries cwd')
    match(cfg.gates['api-test']?.cmd, /npm test/, 'subproject gate cmd')
    eq(cfg.gates.TODO, undefined, 'real gates found → no TODO placeholder')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: detects depth-2 workspace packages via package.json "workspaces" glob', () => {
    const d = work('adopt-workspaces')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ name: 'root', workspaces: ['packages/*'] }))
    mkdirSync(join(d, 'packages', 'web'), { recursive: true })
    mkdirSync(join(d, 'packages', 'utils'), { recursive: true })
    writeFileSync(join(d, 'packages', 'web', 'package.json'), JSON.stringify({ name: 'web', scripts: { test: 'x' } }))
    writeFileSync(join(d, 'packages', 'utils', 'package.json'), JSON.stringify({ name: 'utils', scripts: { build: 'x' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const g = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8')).gates
    eq(g['web-test']?.cwd, 'packages/web', 'web package gate cwd')
    eq(g['utils-build']?.cwd, 'packages/utils', 'utils package gate cwd')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: descends a container dir (apps/*) even without a workspace manifest', () => {
    const d = work('adopt-container')
    mkdirSync(join(d, 'apps', 'admin'), { recursive: true })
    writeFileSync(join(d, 'apps', 'admin', 'package.json'), JSON.stringify({ name: 'admin', scripts: { test: 'x' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const g = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8')).gates
    eq(g['admin-test']?.cwd, 'apps/admin', 'app package gate cwd')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: parses pnpm-workspace.yaml globs, incl. a literal entry with an inline comment', () => {
    const d = work('adopt-pnpm')
    writeFileSync(join(d, 'pnpm-workspace.yaml'), "packages:\n  - 'packages/*'\n  - \"tools/cli\"  # a literal path\n")
    mkdirSync(join(d, 'packages', 'web'), { recursive: true }); mkdirSync(join(d, 'tools', 'cli'), { recursive: true })
    writeFileSync(join(d, 'packages', 'web', 'package.json'), JSON.stringify({ scripts: { test: 'x' } }))
    writeFileSync(join(d, 'tools', 'cli', 'package.json'), JSON.stringify({ scripts: { build: 'x' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const g = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8')).gates
    eq(g['web-test']?.cwd, 'packages/web', 'pnpm glob package')
    eq(g['cli-build']?.cwd, 'tools/cli', 'pnpm literal with inline comment parsed')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: honors a workspace negation glob (excluded package gets no gate)', () => {
    const d = work('adopt-negation')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ workspaces: ['packages/*', '!packages/legacy'] }))
    mkdirSync(join(d, 'packages', 'web'), { recursive: true }); mkdirSync(join(d, 'packages', 'legacy'), { recursive: true })
    writeFileSync(join(d, 'packages', 'web', 'package.json'), JSON.stringify({ scripts: { test: 'x' } }))
    writeFileSync(join(d, 'packages', 'legacy', 'package.json'), JSON.stringify({ scripts: { test: 'x' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const g = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8')).gates
    eq(g['web-test']?.cwd, 'packages/web', 'included package gated')
    eq(g['legacy-test'], undefined, 'excluded package NOT gated (negation honored, incl. container fallback)')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: a leaf-name collision falls back to a full-path-prefixed gate id', () => {
    const d = work('adopt-collision')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ workspaces: ['packages/*', 'apps/*'] }))
    mkdirSync(join(d, 'packages', 'web'), { recursive: true }); mkdirSync(join(d, 'apps', 'web'), { recursive: true })
    writeFileSync(join(d, 'packages', 'web', 'package.json'), JSON.stringify({ scripts: { test: 'x' } }))
    writeFileSync(join(d, 'apps', 'web', 'package.json'), JSON.stringify({ scripts: { test: 'x' } }))
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const g = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8')).gates
    eq(g['web-test']?.cwd, 'packages/web', 'first leaf keeps the short id')
    eq(g['apps-web-test']?.cwd, 'apps/web', 'second collides → full-path id')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: a non-Python subproject with a bare tests/ dir gets NO spurious pytest gate', () => {
    const d = work('adopt-no-pytest')
    mkdirSync(join(d, 'e2e', 'tests'), { recursive: true }) // tests/ but no package.json / pyproject
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    const cfg = JSON.parse(readFileSync(join(cdir(d), 'cadence.config.json'), 'utf8'))
    eq(cfg.gates['e2e-test-py'], undefined, 'no pytest gate from a bare tests/ subdir [S1]')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: survives a subdir with a DIRECTORY named like a marker (no EISDIR crash)', () => {
    const d = work('adopt-eisdir')
    mkdirSync(join(d, 'weird', 'package.json'), { recursive: true }) // package.json is a DIRECTORY
    const r = run('adopt.mjs', [], { dir: d, cwd: d })
    eq(r.status, 0, 'adopt completes without crashing [M1]')
    assert(existsSync(join(cdir(d), 'cadence.config.json')), 'config written (not half-adopted)')
  })

testIf(haveSibling('adopt.mjs'), ADOPT_SRC_ONLY,
  'adopt: writes a .cadence/.gitignore that actually ignores the volatile ledger', () => {
    const d = work('adopt-gitignore')
    writeFileSync(join(d, 'package.json'), JSON.stringify({ name: 'p', version: '1.0.0' }))
    gitInit(d)
    eq(run('adopt.mjs', [], { dir: d, cwd: d }).status, 0, 'exit 0')
    match(readFileSync(join(cdir(d), '.gitignore'), 'utf8'), /loop-state\.json/, 'gitignore lists loop-state.json')
    if (existsSync(join(d, '.git'))) { // prove git honors it (skip if git absent)
      eq(spawnSync('git', ['check-ignore', '.cadence/loop-state.json'], { cwd: d, encoding: 'utf8' }).status, 0,
        'git check-ignore confirms loop-state.json is ignored')
    }
  })

// ── K. overview — the self-contained HTML reporting surface ────────────────────
test('overview: renders a self-contained, dependency-free HTML page with the ledger lanes', () => {
  const d = work('ov-render')
  run('ledger.mjs', ['init', '--goal', 'ship it'], { dir: d })
  run('ledger.mjs', ['add', 'a', '5', 'do a', '--gate', 'test'], { dir: d })
  const r = run('overview.mjs', ['--stdout'], { dir: d })
  eq(r.status, 0, 'exit 0')
  match(r.stdout, /<!doctype html>/i, 'doctype')
  for (const lane of ['pending queue', 'gate signals', 'shipped', 'verified facts', 'cadence overview']) match(r.stdout, new RegExp(lane), `lane: ${lane}`)
  assert(!r.stdout.includes('<script'), 'no <script> — fully static / no external assets')
})

test('overview: HTML-escapes ledger content (no injection from the goal/desc)', () => {
  const d = work('ov-escape')
  run('ledger.mjs', ['init', '--goal', 'ship <b>v2</b> & "go"'], { dir: d })
  const r = run('overview.mjs', ['--stdout'], { dir: d })
  eq(r.status, 0, 'exit 0')
  assert(!r.stdout.includes('<b>v2</b>'), 'raw HTML from a ledger string is NOT injected')
  match(r.stdout, /ship &lt;b&gt;v2&lt;\/b&gt; &amp; &quot;go&quot;/, 'goal is HTML-escaped in the body')
})

test('overview: fails closed on a missing ledger (exit 1)', () => {
  const d = work('ov-missing') // no init → no loop-state.json
  const r = run('overview.mjs', ['--stdout'], { dir: d })
  eq(r.status, 1, 'exit 1'); match(r.stderr, /^✗/, 'error to stderr')
})

test('overview: writes cadence-overview.html into .cadence/ by default', () => {
  const d = work('ov-file')
  run('ledger.mjs', ['init'], { dir: d })
  eq(run('overview.mjs', [], { dir: d }).status, 0, 'exit 0')
  assert(existsSync(join(cdir(d), 'cadence-overview.html')), 'wrote .cadence/cadence-overview.html')
})

test('overview: tick refreshes a pre-existing overview, but never creates one unprompted', () => {
  const d = work('ov-tick')
  run('ledger.mjs', ['init', '--goal', 'x'], { dir: d })
  run('tick.mjs', ['--no-gate'], { dir: d })
  assert(!existsSync(join(cdir(d), 'cadence-overview.html')), 'tick does NOT create the overview unprompted')
  run('overview.mjs', [], { dir: d }) // opt in by generating it once
  const before = readFileSync(join(cdir(d), 'cadence-overview.html'), 'utf8')
  run('ledger.mjs', ['add', 'fresh-item', '5', 'changed the ledger'], { dir: d })
  run('tick.mjs', ['--no-gate'], { dir: d })
  const after = readFileSync(join(cdir(d), 'cadence-overview.html'), 'utf8')
  assert(after.includes('fresh-item'), 'tick refreshed the overview to reflect the new ledger state')
  assert(before !== after, 'overview content changed after the tick')
})

test('overview: a blocked item without `since` renders no dangling label', () => {
  const d = work('ov-since')
  run('ledger.mjs', ['init'], { dir: d })
  const s = readLedger(d)
  s.blockedOnOwner = [{ id: 'x', desc: 'awaiting owner' }] // no `since` (optional in the schema)
  writeRaw(d, 'loop-state.json', JSON.stringify(s, null, 2))
  const r = run('overview.mjs', ['--stdout'], { dir: d })
  eq(r.status, 0, 'exit 0'); assert(!/since t</.test(r.stdout), 'no empty "since t" span')
})

test('overview: -o without a path errors instead of silently defaulting', () => {
  const d = work('ov-noarg')
  run('ledger.mjs', ['init'], { dir: d })
  const r = run('overview.mjs', ['-o'], { dir: d })
  eq(r.status, 2, 'exit 2'); match(r.stderr, /requires a path/, 'message')
  assert(!existsSync(join(cdir(d), 'cadence-overview.html')), 'did not silently write the default')
})
