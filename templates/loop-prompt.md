...
EACH TICK:
1. Run `node .cadence/lib/tick.mjs` — it AUTO-RECONCILES any interrupted prior tick (crash recovery), then prints the ledger digest, the next item, and the relevant gate signals. It also refreshes `cadence-overview.html` if present — a self-contained progress dashboard (run `node .cadence/lib/overview.mjs --open` once to start it; thereafter every tick keeps it current).
2. Take the highest-value pending item. If it has `lastError`, address that error. **If it is `plan-backlog` (or the queue is empty/thin), PLAN instead of executing [P10]:** explore ONCE (retrieval-first), write per-item briefs to `.cadence/plan/<goal>.briefs.md` that pre-resolve exact reads (files + line regions), then `ledger.mjs add <id> <score> "<desc>" --gate <g> --accept "<criterion>" --brief plan/<goal>.briefs.md#<id>` for each, and `done` the plan item. For an EXECUTION item, read its `brief` pointer and read ONLY what it lists.
